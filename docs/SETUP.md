...
- 每 45 分鐘發送一次主提醒。
- 不發送第 5、10 分鐘的追催提醒。
- 每一輪維持約 45 分鐘，下一輪開始時結束上一輪。
- `本輪跳過`只記錄目前回合，下一輪仍會正常提醒。
- 排程暫停時，`立即提醒`會要求管理者先輸入 `開始提醒`。
...
這是目前的預期行為。追催提醒已關閉，Bot 只會每 45 分鐘發送一次主提醒。

### Bot 能自動知道我的時區嗎？

不能。LINE webhook 與群組成員 profile 沒有提供可靠的時區資料；這個版本固定以 `Asia/Taipei`、每日 09:00–18:00 服務台灣使用者。
